 ...
 * the server restarts.
 */
import { Collection, ObjectId, MongoClient, MongoError, Filter } from "mongodb";
...
export async function updateOnePokemon(
	...
	const result = await pokemonCollection?.findOneAndUpdate(
		{ _id: new ObjectId(pokemonId) },
		updatedData as Filter<Pokemon>,
	);
